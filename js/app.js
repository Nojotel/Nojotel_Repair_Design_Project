...
function initSlider() {
    ...
    let sliderImages = document.querySelector('.projects-slider-img')
    let sliderArrow = document.querySelectorAll('.projects-arrow')
    let sliderArrowTel = document.querySelectorAll('.area-arrow')
    let sliderDots = document.querySelector('.projects-circle')
    ...
    let sliderLink = document.querySelector('.projects-slider-elements')
    ...
    function initImages() {
        images.forEach((image, index) => {
            let elementImg = `<img class="projects-img n${index} ${index === 0 ? 'active' : ''}" src="./${image.src}" alt="Project" data-index="${index}">`
    ...
    function initInfo() {
        images.forEach((elem, index) => {
            let elementCity = `<p class="projects-specification-text n${index} ${index === 0 ? 'active' : ''}" data-index='${index}'>${elem.city}</p>`
            let elementApartment = `<p class="projects-specification-text n${index} ${index === 0 ? 'active' : ''}" data-index='${index}'>${elem.apartmentArea}</p>`
            let elementTime = `<p class="projects-specification-text n${index} ${index === 0 ? 'active' : ''}" data-index='${index}'>${elem.repairTime}</p>`
            let elementCost = `<p class="projects-specification-text n${index} ${index === 0 ? 'active' : ''}" data-index='${index}'>${elem.repairCost}</p>`
    ...
    function initLink() {
        images.forEach((image, index) => {
            let link = `<a class="projects-element n${index} ${index === 0 ? 'active' : ''}" href="#" data-index='${index}'>${image.link}</a>`
            sliderLink.innerHTML += link
        })
        sliderLink.querySelectorAll('.projects-element').forEach(link => {
    ...
    function initDots() {
        images.forEach((image, index) => {
            let dots = `<div class="projects-circle-item n${index} ${index === 0 ? 'active' : ''}" data-index='${index}'></div>`
            sliderDots.innerHTML += dots
        })
        sliderDots.querySelectorAll('.projects-circle-item').forEach(dot => {
